# -*- coding: utf-8 -*-
"""
Learning Scout vs Baseline 评估脚本
...
# ====== 手动评分数据 ======
# 格式: (bl_score, ls_score)  1-5分
# 评判依据: 基于答案内容质量、完整性、准确性
# 维度: accuracy, depth, clarity, usefulness, overall
...
scores = {
    ...
    # Q5 RESTful API
    5:  {"accuracy": (4,4), "depth": (4,3), "clarity": (4,4), "usefulness": (4,4), "overall": (4,3)},
    # Q6 Python list/tuple
...
for item in bl_data:
    ...
    for dimension in dimensions:
        ...
        rows.append({
            'Q#': idx,
            '维度': dimension,
            'LS回答摘要': ls_ans[:100] + '...' if len(ls_ans) > 100 else ls_ans,
            'Baseline回答摘要': bl_ans[:100] + '...' if len(bl_ans) > 100 else bl_ans,
            'LS得分': ls_sc,
            'Baseline得分': bl_sc,
            '胜负': result,
            '备注': remark,
...
for dim in dimensions:
    dim_df = df_detail[df_detail['维度'] == dim]
...
for idx in range(1, 34):
    ...
    # P0条件: overall维度LS败 或 accuracy有事实错误
    overall_dim = scores[idx]['overall']
    acc_dim = scores[idx]['accuracy']
    
    if overall_dim[1] < overall_dim[0]:  # LS败
        p0_rows.append({
            'Q#': idx,
            '问题题目': item['question'][:80],
            '问题类型': item.get('dimension', ''),
            '难度': item.get('difficulty', ''),
            'P0原因': f'overall维度LS败({overall_dim[1]}vs{overall_dim[0]})',
            'LS得分': overall_dim[1],
            'Baseline得分': overall_dim[0],
            'LS回答摘要': ls_ans[:150],
            'Baseline回答摘要': bl_ans[:150],
        })
    elif acc_dim[1] < acc_dim[0]:  # accuracy LS败
        p0_rows.append({
            'Q#': idx,
            '问题题目': item['question'][:80],
            '问题类型': item.get('dimension', ''),
            '难度': item.get('difficulty', ''),
            'P0原因': f'accuracy维度LS败({acc_dim[1]}vs{acc_dim[0]})',
            'LS得分': acc_dim[1],
...
for dim in dimensions:
    ...
    if loss_qs:
        improvement_rows.append({
            '优先级': 'P1' if len(loss_qs) > 5 else 'P2',
            '维度': {
                'accuracy': '事实准确性',
                'depth': '深度与完整度',
                'clarity': '表达清晰度',
...
def style_cell(cell, value, align='center', fill=None):
    cell.value = value
...
# 写入表头
headers1 = ['Q#', '维度', 'LS回答摘要(前100字)', 'Baseline回答摘要(前100字)', 
...
for row_idx, (_, row) in enumerate(df_detail.iterrows(), 2):
    ...
    ws1.cell(row=row_idx, column=2).value = row['dimension_full']
    ws1.cell(row=row_idx, column=3).value = row['ls_full'][:100] + ('...' if len(row['ls_full'])>100 else '')
    ws1.cell(row=row_idx, column=4).value = row['bl_full'][:100] + ('...' if len(row['bl_full'])>100 else '')
    ...
    # 胜负颜色
    result = row['胜负']
    fill = win_fill if result == 'LS胜' else (lose_fill if result == 'LS败' else tie_fill)
...
title_cell.value = "Learning Scout vs Baseline 各维度胜负统计汇总"
...
ws2.cell(row=10, column=1).value = "总体结论"
...
ws2.cell(row=13, column=1).value = f"LS平均得分: {df_summary['LS均分'].mean():.2f}/5 | Baseline平均得分: {df_summary['Baseline均分'].mean():.2f}/5"
ws2.cell(row=13, column=1).alignment = left_align
ws2.merge_cells('A13:H13')

# 结论文字
ws2.cell(row=15, column=1).value = "结论:"
ws2.cell(row=15, column=1).font = Font(bold=True, size=11)

if overall_win_rate >= 60:
    conclusion = "LS在多数维度上达到了Baseline的水准，整体表现良好"
elif overall_win_rate >= 50:
    conclusion = "LS与Baseline基本持平，在部分维度有提升空间"
else:
    conclusion = "LS在多数维度上落后于Baseline，需要重点改进"

ws2.cell(row=16, column=1).value = conclusion
...
ws3.cell(row=1, column=1).value = "⚠️ P0问题清单 - 必须立即修复"
ws3.cell(row=1, column=1).font = Font(size=14, bold=True, color="FF0000")
...
if len(df_p0) > 0:
    ...
    for row_idx, (_, row) in enumerate(df_p0.iterrows(), 4):
        ...
        ws3.cell(row=row_idx, column=8).value = f"LS: {row['LS回答摘要'][:80]}... | BL: {row['Baseline回答摘要'][:80]}..."
    ...
    ws3.cell(row=3, column=1).value = "✅ 暂未发现P0级别问题"
...
ws4.cell(row=1, column=1).value = "📋 改进建议清单（按优先级排序）"
ws4.cell(row=1, column=1).font = Font(size=14, bold=True, color="1F4E79")
...
if overall_ls_avg >= overall_bl_avg - 0.3:
    conclusion_text = f"LS整体达到了Baseline的水准（LS均分{overall_ls_avg:.2f} vs Baseline均分{overall_bl_avg:.2f}），在多数题目上回答质量相近，部分题目有差距。"
else:
    conclusion_text = f"LS整体略逊于Baseline（LS均分{overall_ls_avg:.2f} vs Baseline均分{overall_bl_avg:.2f}），主要差距在深度和实用性方面。"
print(conclusion_text)

print("\n### 关键发现")
print(f"- 最严重问题（LS明显弱于Baseline）：深度维度（depth）LS平均得分{scores.get('depth', {}).get(1, (0,0))[1] if 1 in scores else 'N/A'}，部分题目LS回答过于简略")
print(f"- LS明显强于Baseline的方面：表达清晰度（clarity）维度两者持平，LS在结构化表达方面表现良好")
print(f"- 两者相当的方面：事实准确性（accuracy）在大多数题目上表现相当，均能正确回答")
...
for _, row in p1_rows.iterrows():
    print(f"  - {row['维度']}: 涉及{row['LS败题目数']}题（{row['涉及题号']}）- {row['改进建议'][:50]}...")
...
for _, row in df_summary.iterrows():
    ...
    if win_rate >= 60:
        ...
        status = "❌ LS需改进"
    print(f"\n{status} | {dim_name}:")
    print(f"  LS胜率: {row['LS胜率']} | Baseline均分: {row['Baseline均分']} | LS均分: {row['LS均分']}")
